...
public partial class BackgroundScores : System.Web.UI.Page
{
    ...
    //{
    //    if (url.Equals("http://www.subtlegopala.com/Media/BScores/Ilaiyaraja/"))
    //    {
    //        return "<a href=\".*\">(?<A>.*)</a>";
    //    }
    ...
    public static string GetPlaylist(string folderName)
    {
        ...
        string url = "http://www.subtlegopala.com/Media/NonIndianLegends/BScores";
    ...
    public static string GetFileListing(List<string> dirNames)
    {
        ...
        {
            string url = "http://www.subtlegopala.com/Media/NonIndianLegends/BScores/" + dirName;
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            using (HttpWebResponse response = (HttpWebResponse)request.GetResponse())
            {
                using (StreamReader reader = new StreamReader(response.GetResponseStream()))
